...
import com.ratnesh.connecthub.postservice.client.AIServiceClient;
...
public class PostServiceImpl implements PostService {
    ...
    public PostDto createPost(
        ...
        if ((postCreateRequestDto.content() == null
                || postCreateRequestDto.content().isBlank())
        ...
        if (files != null && !files.isEmpty()) {
            ResponseEntity<List<String>> response =
                    uploaderServiceClient.uploadFiles(files);

            if (response.getBody() != null) {
                imageUrls = response.getBody();
            }
        }
    ...
    @Override
    public ImprovePostResponse improvePost(Long postId) {

        Post post = postRepository.findById(postId)
                .orElseThrow(() -> new ResourceNotFoundException("Post",postId.toString()));

        return aiServiceClient.improvePost(
                new ImprovePostRequest(post.getContent())
        );
    }
    ...
    public GeneratePostResponse generatePost(
        ...
        return aiServiceClient.generatePost(request);
    }
    ...
    public CommentSummaryResponse summarizeComments(
        ...
        List<String> comments = commentRepository
                .findByPostId(postId)
                .stream()
                .map(Comment::getContent)
                .toList();

        return aiServiceClient.summarizeComments(
                new SummarizeCommentsRequest(comments)
        );
    }
